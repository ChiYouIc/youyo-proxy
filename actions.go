...
type FuncRespHandler func(resp *http.Response) *http.Response

// FuncRespHandler.Handle(req,ctx) <=> FuncRespHandler(req,ctx)
func (f FuncRespHandler) Handle(resp *http.Response) *http.Response {
	return f(resp)
}
...
type HttpsHandler interface {
	HandleConnect(req string) (*ConnectAction, string)
}

// A wrapper that would convert a function to a HttpsHandler interface type
type FuncHttpsHandler func(host string) (*ConnectAction, string)

// FuncHttpsHandler should implement the RespHandler interface
func (f FuncHttpsHandler) HandleConnect(host string) (*ConnectAction, string) {
	return f(host)
}
